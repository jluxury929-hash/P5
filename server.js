// ===============================================================================
// APEX OMNISCIENT WHALE TITAN v22.0 - HIGH-FREQUENCY CLUSTER
// ===============================================================================
...
const CONFIG = {
    ...
    MARGIN_ETH: "0.012" // ~$40 net profit floor
};
...
if (cluster.isPrimary) {
    ...
    console.log(`${TXT.bold}${TXT.gold}║   ⚡ OMNISCIENT WHALE TITAN | CLUSTER EDITION v22.0    ║${TXT.reset}`);
    console.log(`${TXT.bold}${TXT.gold}╚════════════════════════════════════════════════════════╝${TXT.reset}\n`);
...
async function initWorker() {
    ...
    try {
        ...
        // Upgraded from block scanning to pending scanning for high-frequency execution
        wsProvider.on("pending", async (txHash) => {
            scanCount++;
            process.stdout.write(`\r${TXT.blue}⚡ SCANNING${TXT.reset} | Txs: ${scanCount} | ETH: $${currentEthPrice.toFixed(2)} `);

            // Fetch pending transaction to check Whale Threshold
            // This is more intensive but necessary for "Omniscient" detection before the block is mined
            try {
                const tx = await httpProvider.getTransaction(txHash).catch(() => null);
                
                if (tx && tx.value >= CONFIG.WHALE_THRESHOLD) {
                    process.stdout.write(`\n${TXT.magenta}🚨 WHALE SPOTTED: ${ethers.formatEther(tx.value)} ETH | Hash: ${txHash.substring(0,10)}...${TXT.reset}\n`);
                    await executeOmniscientStrike(httpProvider, signer, titanIface, oracleContract, nextNonce, currentEthPrice);
                }
            } catch (e) {
                // Ignore fetch errors in high-frequency loop
            }
        });
...
async function executeOmniscientStrike(provider, signer, iface, oracle, nonce, ethPrice) {
    try {
        console.log(`${TXT.yellow}🔄 CALCULATING OMNISCIENT VECTOR...${TXT.reset}`);

        // 1. DYNAMIC LOAN SIZING (Binary Logic: Rich vs Poor)
        const balance = await provider.getBalance(signer.address);
        const ethBalance = parseFloat(ethers.formatEther(balance));
        
        // Scale loan: if rich (>0.1 ETH) borrow 100 ETH, else borrow 25 ETH
        const loanAmount = ethBalance > 0.1 ? ethers.parseEther("100") : ethers.parseEther("25");
        
        console.log(`${TXT.dim}⚖️ Leverage Set: ${ethers.formatEther(loanAmount)} ETH (Balance: ${ethBalance.toFixed(4)} ETH)${TXT.reset}`);
